  ...
  getResultFromDB : function(key,retrieveLast){
    ...
    if(result.getSize()>0){
      ...
      if(retrieveLast || current.ttl==undefined || (((new Date()).getTime()-(new Date(current.timestmp)).getTime())/1000)<=current.ttl){
        v=current.data;
        //new ttl if cache is retrieved from db and it is putted in cache
        current.ttl ? this.cache.put(key, v, current.ttl - (((new Date()).getTime()-(new Date(current.timestmp)).getTime())/1000)) : this.cache.put(key, v);
  ...
  get : function(key,retrieveLast){
    ...
    if(cached!=null){
      ...
      if(cached==null){//retry in scriptdb
        Logger.log("trying from splitted item in db " + key + "_" + parseInt(cont))
